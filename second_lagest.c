...
int prepare(int a[], int len)
{
	if (len <= 1) {
		return a[0];
	} else {
		if (prepare(a, len / 2) > prepare(a + len / 2, len - len / 2))
			swap(&a[len / 2 - 1], &a[len - 1]);
		return a[len - 1];
	}
}
...
int find_next(int a[], int len)
{
	if (len < 2) {
		...
		int mid = (len + 1) / 2;
		if (a[mid - 1] < a[len - 1])
			return find_next(a + mid, len - mid);
		else
			return find_next(a, mid);
	}
}

int second_lagest(int a[], int len)
{
	int i;
	prepare(a, len);
	return find_next(a, len - 1);
}
